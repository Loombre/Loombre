...
├── polkit/
│   └── 50-loombre.rules          # -> /usr/share/polkit-1/rules.d/ — wheel/sudo/admin manage loombre-*.service
│                                 # with their OWN password (the tray's Start / Shut down); JS-rules polkit only
├── desktop/                      # desktop integration for bin/loombre-tray (installers/linux/desktop):
│   ├── loombre.desktop.template            # -> /usr/share/applications/loombre.desktop (app-menu launcher)
│   ├── loombre-tray-autostart.desktop.template  # -> /etc/xdg/autostart/loombre-tray.desktop (login autostart)
│   └── icons/hicolor/<N>x<N>/apps/loombre.png + scalable/apps/loombre.svg  # -> /usr/share/icons/hicolor/...
│                                 # (icons generated by scripts/build-linux-icons.mjs, committed; rendered/copied
│                                 # by install.sh with the same __PREFIX__ sed as systemd/, and by the .rpm/.deb)
├── bin/
...
│   ├── loombre-ipc-dir-setup     # loombre-server.service's ExecStartPre=+ step (root): re-groups /run/loombre to the
│   │                               # admin group + setgid 2750 so the IPC files the server writes are tray-readable
│   └── loombre-tray              # the desktop tray controller: a single static Go binary (installers/linux/tray,
│                                   # cross-compiled by build-tarball.mjs with CGO_ENABLED=0; the Linux counterpart of
│                                   # the macOS menubar app / Windows tray — see installers/linux/tray/README.md)
├── runtime/
│   └── node/
...
| Path | Produced by |
|------|-------------|
| `bin/` (wrappers), `install.sh`, `uninstall.sh`, `systemd/`, `desktop/`, `loombre.env.template`, `VERSION` | this build script, generated/copied directly |
| `bin/loombre-tray` | `installers/linux/tray` — `go build` (static, `GOARCH` from `--arch`; `--skip-tray` for a tray-less local build) |
...
When `LOOMBRE_DATA_DIR` is set (every installed shape), the service
wrappers also default `LOOMBRE_TRANSCODE_DIR` to `<data dir>/transcode`:
the units run with `PrivateTmp=true`, so the worker's `os.tmpdir()`
default was a `/tmp` the server could never see (every HLS session sat at
"loading"). The server unit sets `LOOMBRE_IPC_DIR=/run/loombre`, and
`bin/loombre-server` defaults the token's group (`LOOMBRE_IPC_GROUP`) to
that directory's own group. Who sets that group: `bin/loombre-ipc-dir-setup`,
the server unit's `ExecStartPre=+` step, run as root before the server
starts — it creates `/run/loombre` owned by the service account,
re-groups it to the host's local-administrator group (first of
`wheel`/`sudo`/`admin`, or `LOOMBRE_IPC_GROUP`) and makes it setgid
`2750`, so the server's `0640` files inherit the group and
`bin/loombre-tray`, running as an admin console user, can read them
(`ExecStopPost=+ … --remove` deletes it on stop). Root has to do the
chgrp: an unprivileged process may only hand a file to a group it belongs
to, and `loombre` is not an administrator. Not a `RuntimeDirectory=`:
systemd re-applies user/group/mode to one at ExecStart setup, after
ExecStartPre, which undid the grant on the reference box. Explicit
env-file values win in every case.
...
(`loombre` by default) and adds it to the `render`/`video` groups that
exist (GPU device access for hardware transcoding — Intel Quick Sync and
VAAPI open `/dev/dri/renderD*`), copies this payload to `/opt/loombre`
(default, `--prefix` to change), renders `desktop/`'s two `.desktop`
templates into `/usr/share/applications` + `/etc/xdg/autostart` and copies
the hicolor icons (best-effort, never fatal), creates `/var/lib/loombre` (default, `--data-dir`)
...
/usr/share/applications/loombre.desktop           # app-menu launcher (rendered from desktop/loombre.desktop.template)
/etc/xdg/autostart/loombre-tray.desktop           # tray login autostart (rendered from desktop/loombre-tray-autostart.desktop.template)
/usr/share/icons/hicolor/<N>x<N>/apps/loombre.png # + scalable/apps/loombre.svg — explicit files (the size dirs belong to hicolor-icon-theme)
/usr/share/polkit-1/rules.d/50-loombre.rules      # the polkit rule above, verbatim (rules.d belongs to polkit)
...
**What is NOT in a package.** `install.sh`, `uninstall.sh`, `systemd/`,
`desktop/`, `polkit/` and `loombre.env.template` are tarball-channel files and never
ship inside a package (`TARBALL_ONLY_ENTRIES`) — the package *is* the
installer, and its templates are rendered at package-build time (the
desktop entries and icons land in their FHS locations as real files). The writable Next
