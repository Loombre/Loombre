...
below — `1.0.0-beta.3` is the current release, a pre-release for
...
to GHCR. Note the tag form: git tags carry a leading `v` (`v1.0.0-beta.3`),
but the published image tags are the bare semver (the release pipeline
strips the `v`). The current release is `1.0.0-beta.3` — a **pre-release
