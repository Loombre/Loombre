...
| P11 | Adult/sensitive content unsupported (LS hides it poorly; PS prohibits it) | Never a design goal | Native restricted content class: opt-in, PIN-gated, server-enforced, metadata-isolated (§6.4) |
...
- **`media_files`** — path, `content_hash`, size, container, duration,
  ...
  (4K/1080p) handled natively, another PS/LS sore spot.
...
the anti-LS choice for music correctness).
...
per field, not per item — finer than LS). Image ingest: original cached →
