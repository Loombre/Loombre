...
<!-- Provenance (kept out of the rendered page deliberately — F24-3):
     authored as Phase 4 Wave 3, deliverable H; STATE.md Phase 4 Mission:
     "a physical-hardware Tier-0 performance audit". STATE.md D15/P2.6
     record the T0 perf harness this runbook builds on. The authoring
     lane's no-hardware verification notes that used to render as this
     page's §10 now live in a comment above the Appendix, below. -->

**This runbook is executed by hand, by the server's owner, on real Intel
N100 hardware** — no CI or dev environment can substitute for it
(T0-throttled CI containers approximate CPU/RAM limits; they cannot
approximate a real QSV media engine, a real spinning HDD, or 30 minutes of
sustained real silicon). This document plus `scripts/t0-audit/**` make
that execution turnkey and its pass/fail criteria mechanical; **you still
have to physically run it.**
...
budgets), §9.3 (web client budgets). The T0 perf harness this runbook
builds on (`scripts/perf-t0.mjs`) is already CI-enforcing on every push.
Nothing in this runbook changes those budgets — it only measures against
...
target, use ports 4100–4199 (a range reserved for this runbook's own
smoke-testing; nothing else in this repo's tooling uses it) so you don't
collide with anything else running.
...
This audit requires embedded PostgreSQL specifically — this is the
packaging path public installs actually ship, and it's the one with a
real "cold start includes provisioning" story worth measuring. The
external-PG path (`docs/ops/external-postgres.md`) is equally
first-class, but it is **not** what this audit measures.
...
# REQUIRED for this audit's preflight, though the install itself no longer
# needs it: the tarball's server wrapper (bin/loombre-server, generated by
# installers/linux/build-tarball.mjs) already exports this automatically
# at runtime when DATABASE_URL is unset. scripts/t0-audit/preflight.mjs,
# however, verifies the env FILE — a wrapper-side runtime export doesn't
# satisfy that read — and an explicit line here also pins exactly which PG
# payload this audit measured:
LOOMBRE_EMBEDDED_PG_VENDOR_DIR=/opt/loombre/pg

# Strongly recommended (avoids the ephemeral-JWT-secret restart-logs-
# everyone-out footgun):
...
setup wizard for this instance** (the setup surface is inert once any
user exists) — expected and correct for this audit (the wizard flow has
its own separate coverage in the install smoke tests), but don't also try
to click through onboarding on this same install afterward expecting it
to work.
...
(a per-backend capability table from the hwprobe output) — the Linux
hardware-verification checklist, adjacent to but not part of this
runbook; doing it now while hwprobe is already fresh in your terminal is
convenient, not required by this runbook.
...
The enforcing CI job (`perf-web-budget`/`perf-lighthouse`) already runs
these, blocking, on every push. What this step adds is running the
**exact same commands** on the
...
<!-- Authoring-verification provenance (formerly rendered as this page's
     §10 — relocated into this comment per F24-3; internal record, not
     operator guidance). No CI/dev host (including the M3 Max this was
     authored on) can run the hardware-dependent paths for real — no
     systemd unit named loombre-server, no /dev/dri, no embedded-PG data
     directory matching the real install layout, no QSV. Verified locally
     without fabricating a hardware result: every script `node
     ‑‑check`-clean; scripts/t0-audit/run-all.sh shellcheck-clean;
     scripts/t0-audit/lib/common.mjs's platform-independent helpers
     (parseArgs, readEnvFile, fmtMiB, readThermalZones/readCpuFreqSample/
     readDmesgThrottleLines on a non-Linux host — degrade to empty
     results/a clear "unavailable" marker rather than throwing) exercised
     directly via `node -e` against real inputs;
     ...
     deliberate-breach run — every MEASURE: placeholder in a scratch copy
     of reports/t0-audit.md stamped with the right value AND the right
     PASS/FAIL verdict in both cases, missing artifacts leaving their
     placeholders untouched with a named warning instead of silently
     defaulting to PASS; sustained-monitor.mjs's internal
     parseSegmentUris/segmentSortKey hand-verified against a real-shaped
     fixture playlist matching the contract's documented runN/sNNNNNN.m4s
     / runN/init.mp4 naming. Not verifiable off-hardware, genuinely
     N100-only: rss-sample.mjs and cold-start.mjs's systemctl/ps calls;
     embeddedPgRssBytes/findFfmpegPidForSessionDir's process discovery;
     the entire live HTTP flow in dual-transcode.mjs/sustained-monitor.mjs
     (needs a real running server + real hardware transcode);
     preflight.mjs's group-membership and capability-report checks; the
     actual §9.2/§9.1/§9.3 pass/fail outcomes themselves — which is
     exactly why this document exists as an owner-run runbook rather than
     a CI job. -->
