...
| ACME http-01 challenge listener (`apps/server/src/tls/acme/http01-server.ts`, port 80) | Pre-existing ACME machinery, now also reachable via the Direct path's staged `POST /admin/remote/direct/acme-test` (RG12): the certificate authority must reach `/.well-known/acme-challenge/...` on port 80 to validate the domain. | Serves ONLY the challenge path with the CA-supplied key-authorization token; up transiently for the issuance window (staged test or renewal), not a standing surface; no Loombre data or identity. The admin op that triggers it is itself live-isAdmin. |
