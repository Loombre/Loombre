...
Runs every workspace package's test suite via Turborepo. DB-backed suites
never touch the database you just set up in step 2: each resolves its own
...
`lint` → `typecheck` →
`test` → `playback-matrix` → `installers-test` → `scripts-test` → `db:migrate-check` →
`grep-gates` →
...
`pnpm gate:full` runs everything `pnpm gate` runs, plus a 19th step,
