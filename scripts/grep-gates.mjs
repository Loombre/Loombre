 ...
 * "no Jellyfin/Emby API surface, schema, or naming anywhere").
 *
 * (a) Forbids "jellyfin"/"emby" (case-insensitive) and the whole word "Ticks"
 *     (case-sensitive — Jellyfin/Emby's tick-based timestamp naming) inside
...
const NAMING_PATTERNS = [
  // "emby" is bounded on both sides so ordinary identifiers that merely
  // contain the letter run (getItemById → "...temBy..." case-insensitively)
  // don't false-positive; real product naming ("emby", "emby-api", "Emby.X")
  // still hits.
  { code: "jellyfin-or-emby", pattern: /jellyfin|(?<![a-z0-9])emby(?![a-z0-9])/i },
