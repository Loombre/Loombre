...
export interface paths {
    ...
    "/admin/items/{id}/apply-match": {
         ...
         * @description Enqueues the EXISTING 'metadata' job with a forced provider ref — re-fetches provider details + artwork for exactly the chosen candidate (bypassing search/pickBestMatch) and merges them through the same field-precedence engine every scan-triggered metadata job already uses (existing nfo/tag-sourced fields keep their precedence unchanged). Never touches the original media file. `provider` is validated against the set this server can actually resolve BEFORE anything is enqueued: an unknown built-in name, or an `lpp:<pluginId>` whose plugin is missing or disabled, is a 422 naming the field and leaves NO job behind (previously any string was accepted with a 202 and the resulting job completed having changed nothing). Item existence is checked first, so a nonexistent item 404s regardless of the body.
         */
