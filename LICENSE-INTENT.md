...
| libxml2 2.9.13 shared library (Linux only, `libxml2.so.2`, staged beside the embedded PostgreSQL's own libraries) | MIT (libxml2's Copyright file ships beside it as `LICENSE.libxml2.txt`) | `installers/libxml2-manifest.json` — the Rocky Linux 9.6 vault's `libxml2` rpm (RHEL 9 rebuild; glibc 2.34 floor, the same as the PostgreSQL binaries), sha256-pinned, fetched by `scripts/fetch-libxml2.mjs`; only the library and its license text are extracted | Dynamically linked into the PostgreSQL executables via their RUNPATH (`$ORIGIN/../lib`) — never loaded by Loombre's own process. Vendored because libxml2 2.14 bumped its soname to `.so.16` and Ubuntu 25.10 / 26.04 LTS ship no `libxml2.so.2` for PostgreSQL to find |
