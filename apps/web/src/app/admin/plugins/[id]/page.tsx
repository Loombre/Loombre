...
//
// LPP v1, Lane W5: the admin Plugin detail page — manifest summary (every
// declared capability + its household-register scope line, same
// CapabilityCard the registration wizard's confirmation screen uses),
// config edit (PluginConfigForm, same auto-rendered widget renderer,
// requireAllSecrets=false so a blank secret field leaves the existing
// keyring value alone), the event-grant editor (only shown when
// event-subscriber is among this plugin's granted capabilities),
// enable/disable, the refresh -> scope-diff -> re-approval flow, HMAC
// rotation (once-display, same secret-box pattern the wizard's result step
// uses), and remove (window.confirm — the established destructive-action
// convention across every other admin page, see admin/users/page.tsx).
//
// Lane W5b adds two more event-subscriber-only sections (same
// `grantedCapabilityTypes.includes("event-subscriber")` gate
// EventGrantsSection already uses): DeliveryStatusSection (GET's additive
// `deliveryStatus`, lib/plugin-delivery-status.ts's pure copy) and
// PseudonymizationSection (PUT .../pseudonymization, register-language
// caution on the "send real ids" direction only — turning protection back
// ON needs no confirmation, only turning it off does, same asymmetric-
// friction posture handleRemove/handleRotateHmac's window.confirm calls
// already establish for consequential-but-not-symmetric actions).
//
// Live updates: subscribes to the same 6 ADMIN_ONLY plugin.* events as the
// list page and refetches — a second open admin tab always converges.
...
/** Lane W5b: `deliveryStatus` panel — event-subscriber plugins only (the
 *  detail page's own gate below matches EventGrantsSection's). Recomputes
 *  its relative-time copy against `Date.now()` on every render rather than
...
/** Lane W5b: the pseudonymization toggle — event-subscriber plugins only.
 *  Default ON (pseudonymizeActorIds:true): activity sent to this plugin
 *  names an anonymous per-plugin id, never a real account. Turning it OFF
 *  needs an explicit confirmation naming what changes (window.confirm,
 *  same friction-on-the-more-exposing-direction posture handleRemove/
...
export default function AdminPluginDetailPage({ params }: { params: Promise<{ id: string }> }): React.JSX.Element {
  const { id } = use(params);
