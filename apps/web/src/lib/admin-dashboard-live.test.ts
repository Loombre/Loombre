...
describe("getLibraryScanState", () => {
  it("returns the idle default for a library with no entry", () => {
    ...
    expect(getLibraryScanState(map, "missing-library")).toEqual({ scanning: false, filesProcessed: null });
  });

  it("returns the live entry when present", () => {
    const map = new Map<string, LibraryScanState>([["lib-1", { scanning: true, filesProcessed: 42 }]]);
    expect(getLibraryScanState(map, "lib-1")).toEqual({ scanning: true, filesProcessed: 42 });
  });
});
