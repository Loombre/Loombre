...
//
// Addendum A, lane S3 (STATE.md, A3/AD1 read-site migration): the four
...
// statement dependency-cruiser's graph walk sees. Splitting the four
...
//
// Every module that used to get these four providers via CommonModule
// alone (catalog.module.ts, playback.module.ts, session.module.ts) now
// imports this module ALONGSIDE CommonModule — see each of those files'
// own updated header comments. Also re-exports SettingsModule itself so a
// consuming module's OWN local providers (e.g. playback's
// PlaybackSessionSweeperService, session's AuthRateLimiterService/
// AnomalyLogService) can inject SettingsService directly without a
// separate explicit SettingsModule import.
...
@Module({
  ...
  providers: [ViewerContextProvider, UpdateCheckService, SurfaceRateLimiterService, SurfaceRateLimitGuard],
  exports: [ViewerContextProvider, UpdateCheckService, SurfaceRateLimiterService, SurfaceRateLimitGuard, SettingsModule],
})
