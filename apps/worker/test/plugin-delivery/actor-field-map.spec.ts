...
describe("ACTOR_FIELD_MAP exhaustiveness over the REAL envelope enum", () => {
  ...
  it("envelope.schema.json enumerates exactly 34 types (15 pre-LPP + 6 plugin.* + 2 watchlist.* [L3] + 1 metadata.match-candidates [L2], Phosphor Wave 2 + 1 user.restricted-pin-reset [H2] + 1 probe.failed [owner ledger L1] + 1 stash.provider.disabled [Stash SQLite metadata sync, S3/K12] + 2 stash.sync.* [Stash SQLite metadata sync, S8/K12, Lane C] + 1 mail.failed [optional mail transport run, E6/M6] + 3 user.invited/user.invite-revoked/user.claimed [E2, Lane A] + 1 user.password-reset [E3/M14/M15, Lane B])", () => {
    expect(realTypes).toHaveLength(34);
