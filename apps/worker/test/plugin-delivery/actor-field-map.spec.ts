...
describe("ACTOR_FIELD_MAP exhaustiveness over the REAL envelope enum", () => {
  ...
  it("envelope.schema.json enumerates exactly 47 types (15 pre-LPP + 6 plugin.* + 2 watchlist.* [L3] + 1 metadata.match-candidates [L2], Phosphor Wave 2 + 1 user.restricted-pin-reset [H2] + 1 probe.failed [owner ledger L1] + 1 stash.provider.disabled [Stash SQLite metadata sync, S3/K12] + 1 stash.provider.connected [Stash OPEN ledger item 7] + 2 stash.sync.* [Stash SQLite metadata sync, S8/K12, Lane C] + 1 mail.failed [optional mail transport run, E6/M6] + 3 user.invited/user.invite-revoked/user.claimed [E2, Lane A] + 1 user.password-reset [E3/M14/M15, Lane B] + 1 session.revoked-by-password-change [Current-password re-auth on self-changes, G5] + 2 notice.published/notice.cancelled [admin broadcast notifications — system notices, N2/NG1, Lane A] + 9 remote.*/tunnel.connector.state/posture.*/probe.arrived [Loombre Remote — embedded WireGuard + three-path wizard + reachability proof + posture card, R9, Wave 0])", () => {
    expect(realTypes).toHaveLength(47);
