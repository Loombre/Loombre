 ...
 *
 * Edition: a Plex-style `{edition-...}` brace wins if present anywhere in
...
export function parseMoviePath(relPath: string): MovieGuess | null {
  ...
  // Plex `{edition-...}` braces are stripped from whichever source string
  ...
  if (edition) reasons.push("edition:plex-brace");
