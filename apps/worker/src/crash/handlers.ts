...
// Loombre :: apps/worker/src/crash/handlers.ts
//
...
// for the same reason apps/server/src/crash/handlers.ts documents: crash
// handlers should be installed as early as possible (before
// waitForDatabaseReady() even runs, so a crash during boot itself still
// produces a crash file), while graceful-shutdown needs index.ts's
// existing shutdown() closures (queue.stop()/hashPool.terminate()/
// watcherHandle.stop()/db.destroy()) built first.
//
// index.ts previously registered SIGINT/SIGTERM directly inline (STATE.md
// P4.14's own audit finding: "worker has SIGINT/SIGTERM only" — no
// SIGBREAK, the Windows service host's I3 gap this module also closes).
// This file REPLACES that inline pair with installGracefulShutdown(),
// same shape as apps/server's new signal handling, so both apps' process-
// level behavior is symmetric and independently testable.
