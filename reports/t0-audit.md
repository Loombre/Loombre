...
**Known architecture gap (STATE.md Phase 4 Open, not this lane's to fix)**:
nothing currently serves the packaged tarball's `web/` output as part of an
installed deployment (no `output: standalone`, no static serving in
apps/server) — see STATE.md's "Web-serving architecture unresolved for
installed deployments" entry. Rows 9–10 are therefore run from a full
source checkout **on the N100's own hardware** (`pnpm run build && pnpm run
start`, real CPU/thermal conditions, real N100), which correctly answers
"does this app hit its web budgets on T0-class hardware" — it does NOT yet
answer "does the shipped installer's web output get served," because
nothing serves it yet. That second question is out of scope for this lane
and stays logged as the existing Open item.
